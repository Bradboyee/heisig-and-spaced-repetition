...
import com.example.finalproject.valueformatter.BarChartFormatter
...
data class BarChartModelWrong(
    ...
    private fun getData(): ArrayList<BarEntry> {
        ...
        for (i in spaced.indices) {
            val barEntry =
                BarEntry(i.toFloat(), spaced[i].status.wrong.toFloat(), spaced[i].kanji)
